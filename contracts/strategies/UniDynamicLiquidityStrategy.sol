...
contract UniDynamicLiquidityStrategy is GovIdentity {
    ...
    function swapETHByCurve(uint256 tokenId,uint256 amountIn)public onlyAuthorize{
        uint256 amountOut=CurveSwapLibrary.swapByPool(tokenId,1,0,amountIn);
        IWETH(WETH).deposit{value: amountOut}();
    }

    function swapByCurve(uint256 tokenId,int128 i,int128 j,uint256 amountIn)public onlyAuthorize{
        CurveSwapLibrary.swapByPool(tokenId,i,j,amountIn);
    }
    ...
    function _withdrawOfUnderlying(address _to,uint256 _amount)internal{
        ...
        for(uint256 i=_pools.length();i>0;i--){
            ...
            uint256 ratio=liquidityValue.mul(P).div(assets);
            uint256 needAmount=ratio.mul(_amount).div(P);
    ...
    function extractableUnderlyingNumber(uint256 _amount)public view returns(uint256[] memory tokenNumbers){
        ...
        for(uint256 i=0;i<_tokens.length();i++){
            if(tokenTotalNumbers[i]>0){
                ...
                uint256 ratio=ta.mul(P).div(assets);
                uint256 needVaultAmount=ratio.mul(_amount).div(P);
