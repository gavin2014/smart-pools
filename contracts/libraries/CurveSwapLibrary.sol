...
library CurveSwapLibrary{
    ...
    function swapByPool(uint256 tokenId,int128 i,int128 j, uint256 amountIn)internal returns(uint256){
        synthSwap.withdraw(tokenId,amountIn);
        ERC20Helper.safeApprove(seth,curve_seth_pool,amountIn);
        return ICurveFi(curve_seth_pool).exchange(i, j, amountIn, 0);
    }
}
